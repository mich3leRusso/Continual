...
from utils.transforms import to_tensor_and_normalize, default_transforms, default_transforms_core50, \
    to_tensor_and_normalize_core50, default_transforms_TinyImageNet, to_tensor_and_normalize_TinyImageNet, \
    default_transforms_Synbols, to_tensor_and_normalize_Synbols, to_tensor
...
def main():
    ...
    if args.dataset == 'CIFAR100':
        ...
        if args.number_perturbations is not None: #perturbe the test set
    ...
    for i, train_taskset in enumerate(strategy.train_scenario):
        ...
        if not args.self_distillation:
            ...
            strategy.pruner.set_gating_masks(strategy.fresh_model, strategy.experience_idx,
                                             weight_sharing=args.weight_sharing, distillation=strategy.distillation)
        ...
        strategy.scheduler = torch.optim.lr_scheduler.MultiStepLR(strategy.optimizer, milestones=args.scheduler,
        ...
        if not args.load_model_from_run:
            with torch.no_grad():
                strategy.pruner.prune(strategy.model, strategy.experience_idx, strategy.distill_model,
                                      args.self_distillation)

        strategy.train_epochs = args.epochs_distillation
        strategy.distillation = True
        strategy.optimizer = torch.optim.AdamW(strategy.model.parameters(), lr=args.lr_distillation,
                                               weight_decay=args.wd_distillation)
        strategy.scheduler = torch.optim.lr_scheduler.MultiStepLR(strategy.optimizer,
                                                                  milestones=args.scheduler_distillation, gamma=0.5,
                                                                  last_epoch=-1, verbose=False)
        print(f"    >>> Start Finetuning epochs: {args.epochs_distillation} <<<")
        strategy.pruner.set_gating_masks(strategy.model, strategy.experience_idx, weight_sharing=args.weight_sharing,
                                         distillation=strategy.distillation)
        if args.load_model_from_run == '':
            print("Start training")
            strategy.train()
        ...
        for idx, temperature in enumerate(args.temperature):
            ...
            if args.number_perturbations is not None:
                ...
                for n_pert in args.number_perturbations:
                    ...
                    if sanity_check:
                        confusion_mat = test_robustness_OOD(strategy, strategy.test_scenario[:11], i, temperature,
                                                            sanity_check)
