...
class GatedCifarResNet(nn.Module):
    ...
    def set_output_mask(self, exp_idx, classes_in_this_exp):
        ...
        if args.mode != 4:
            self.output_mask[exp_idx] = torch.nn.functional.one_hot(torch.tensor(classes_in_this_exp),
                                                                    num_classes=OPT.n_classes).sum(dim=0).float().to(
                OPT.device)
        else:
            self.output_mask[exp_idx] = torch.nn.functional.one_hot(torch.tensor(classes_in_this_exp),
                                                                    num_classes=OPT.n_classes + args.extra_classes * args.n_experiences).sum(
                dim=0).float().to(OPT.device)
    ...
    def save_bn_params(self, task_id):
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.BatchNorm2d):
                bn_params.append(
                    (m.weight.data.clone(), m.bias.data.clone(), m.running_mean.clone(), m.running_var.clone()))
...
def gresnet32_2(dropout_rate=0.1, classes_out=OPT.n_classes):
