...
def get_all_core50_data(path, n_tasks, split):
    ...
    for tid, obj_group in enumerate(tasks):
        ...
        for i, oid in enumerate(obj_group):
            ...
            #indicizza il numero reale della classe
            y = np.stack([tid*len(obj_group)+i for x, y in dset], axis=0)
            #crea un vettore con la stassa lunghezza di y , e che ha come valore il task
            t = np.ones_like(y)*tid
            #append per ciascuna classe
            task_data.append(x)
            task_lbl.append(y)
            task_tlbl.append(t)
        #concatena gli elementi di ciascun task
